...
> columnIxs cx = zip [cx, cx, cx] coordinates
...
> emptyIxs b = filter (\x -> (emptyAt b x) == True) allIxs
...
> won b x = any (\y -> all (\z -> playerAt b x z) y) winLines  
...
> moveScore b x i = negate (minimax (write i x b) (opponent x))
